...
class ACAIE_Implicit_Problem(Implicit_Problem):
    ...
    def res(self, t, y, yd):
        ...
        return np.array([
            ...
            spec_rhs[7] - yd[7], spec_rhs[8] - yd[8], spec_rhs[9] - yd[9]
        ])
...
def dae_rhs(system):
    def dae_array(t, conc):
        ...
        return np.array([
            ...
            (-1 * c.k_app(conc[9]) * conc[4] * conc[6]) +
            (int(t <= system.dose_time_sec) * dosage) -
            (c.k_H2O2(conc[9]) * conc[4] * conc[8]),
            # Fe(II), gen - conversion to Fe(III)
            c.k_app(conc[9]) * conc[4] * conc[6] + (c.k_H2O2(conc[9]) *
                                                    conc[4] * conc[8]),  # Fe(III), pure generation
            c.k_r * (c.O2_saturation_25C - conc[6]) -
            (c.k_app(conc[9]) * conc[4] * conc[6]),
            # O2, mass transfer from air - removal by Fe
            (c.beta * c.k_app(conc[9]) * conc[4] * conc[6]
             ) / (1 + (c.k_1_div_k_2(conc[9]) * conc[4] / conc[0])),
            # As(V) tot = As(III) oxidized
            (int(t <= system.dose_time_sec) * dosage) -
            (c.k_H2O2(conc[9]) * conc[4] * conc[8]),  # H2O2 derivative
            0  # pH, approximate
        ])
    return dae_array
...
def initial_derivative(system):
    ...
    return np.array([
        ...
        init_dae_rhs[8],  # time derivative of H2O2, approximate
        init_dae_rhs[9]   # time derivative of pH, approximate
    ])
